...
data NumeralsAlgorithm = NumeralsAlgorithm {
  ...
  , simpleClockText :: ClockSegment -> DaySegment -> Int -> Int -> Text  -- ^ A function that converts the clock segment and day segment to a /Text/ that describes the time of the day in words.
  }
...
instance NumToWord NumeralsAlgorithm where
    ...
    toTimeText' alg h m = simpleClockText alg (toClockSegment m) (toDaySegment h) h m
...
numeralsAlgorithm :: (Foldable f, Foldable g) => Text -> Text -> Text -> f Text -> g (Integer, Text) -> FreeValueSplitter -> FreeMergerFunction -> (Text -> Text) -> FreeNumberToWords -> (ClockSegment -> DaySegment -> Int -> Int -> Text) -> NumeralsAlgorithm
